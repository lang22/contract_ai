...
# 四类合同模板文档所在路径Unexpected identifier
from config import Config

project_path = os.path.dirname(__file__)
template_path = Config.CONT_GENA_TEMPLATE_DIR_PATH

template_docx_list = ['债权转让协议_单户收购_双方签署.docx',
                      '债权转让协议_应收账款债权.docx',
                      '债权转让协议_我司对外转让.docx',
                      '债权转让协议_打包收购.docx',
                      '债权转让协议_委托贷款债权四方签署.docx',
                      '债权转让协议_工程款债权.docx',
                      '债权转让协议_委托贷款债权三方签署.docx',
                      '债权转让协议_一般企业债权.docx',
                      '债权转让协议_信托公司信托资金信托贷款债权.docx',
                      '债权转让协议_信托公司自有资金信托贷款债权.docx',
                      '债权转让协议_信托计划下回购特定资产形成债权.docx',
                      '债权转让协议_单户收购_三方签署.docx']

template_docx_dict_index = {'01': ('01_1', '01_2', '01_3'),
                            '02': ('02_1', '02_2', '02_3', "02_4", '02_5')}

template_docx_dict = {
    '01_1': '债权转让协议_我司对外转让',
    '01_2': '债权转让协议_打包收购',
    '01_3': '债权转让协议_单户收购_双方签署',

    '02_1': '债权转让协议_应收账款债权',
    '02_2': '债权转让协议_委托贷款债权四方签署',

    '02_3': '债权转让协议_工程款债权',
    "02_4": '债权转让协议_委托贷款债权三方签署',
    '02_5': '债权转让协议_一般企业债权'
}

template_docx_name_dict = {
    '01_1': '债权转让协议（我司对外转让）',
    '01_2': '债权转让协议（打包收购）',
    '01_3': '债权转让协议（单户收购双方签署）',

    '02_1': '债权转让协议（应收账款债权）',
    '02_2': '债权转让协议（委托贷款债权四方签署）',
    '02_3': '债权转让协议（工程款债权）',
    "02_4": '债权转让协议（委托贷款债权三方签署）',
    '02_5': '债权转让协议（一般企业债权）'
}

template_docx_content_dict = {
    '01_1': "适用于我司债权打包/单户对外转让项目。",
    '01_2': "适用于收购银行打包债权。",
    '01_3': "适用于收购银行单户债权资产项目。",

    '02_1': "适用于因供货或提供服务而形成的应收账款债权。",
    '02_2': "适用于委托贷款债权四方签署。",
    '02_3': '适用于工程款债权。',
    "02_4": '适用于委托贷款债权三方签署。',
    '02_5': '适用于一般企业债权。'
...
txt_save_path = Config.CONT_GENA_TEMPLATE_DIR_PATH

# 新生成的模板文档保存路径
filled_docx_path = Config.CONT_GENA_DIR_PATH
...
def output_elements_para_number(template_filename):
    ...
    for str_i in range(docx_string_list_lens):
        ...
        y_res = string.find('月')
        r_res = string.find('日')
        if j_res != -1 and j_res < n_res and j_res < y_res and j_res < r_res and elements_para_number_dict[
            '交易基准日'] == '':
    ...
    for item in elements_para_number_dict:
        ...
        # print(item,'  ',elements_para_number_dict[item])
        # if elements_para_number_dict[item] != '':
        #     print(docx_text_string_list[elements_para_number_dict[item]])
        #     print(document.paragraphs[elements_para_number_dict[item]].text)
...
def save_para_num_txt():
    for item in template_docx_list:
        ...
        # print(item)
        # for item_ in elements_para_num_dict:
        #     print(item_,'  ',elements_para_num_dict[item_])
...
# 输出：填充的一个新的合同文档
def contract_generation(choose_template_name, elements_dict, contract_filename='contract_generated'):
    ...
    print('contract_generation:', template_docx_path)
    # 开始对合同模板进行填充
    # 传入的模板文件路径存在
    if template_docx_path != '':
        ...
        if elements_para_number_dict['债权本息总额'] != -1:
            ...
            template_string_filled = template_string[
                                     :loc + 1] + zq_dx_string + '元（小写：人民币' + zq_xx_string + '元），' + '其中本金余额人民币' + bj_dx_string + '元（小写：人民币' + bj_xx_string + '元），' + '欠息人民币' + qx_dx_string + '元（小写：人民币' + qx_xx_string + '元）。'
        ...
        # for item in elements_para_number_dict:
        #     if elements_para_number_dict[item] != -1:
        #         print(document.paragraphs[elements_para_number_dict[item]].text)
        # print('\n'*3)

        # 保存填充后的合同文档
        filled_docx_filename = contract_filename + '.docx'
