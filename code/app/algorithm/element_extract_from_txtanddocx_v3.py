...
from app.tools.check_amount import check

from elem_pickup import pikeup

jieba.setLoglevel(jieba.logging.INFO)

check_amount = check

basepath = Config.STATIC_ELEM_MODEL

zhuanrangfang_obj_file_path = os.path.join(basepath, '转让方_NLPCLSModel.obj')
zhuanrangfang_model_file_path = os.path.join(basepath, '转让方_LR_CO_OCCURRENCE_MATRIX_plus_0.model')

shourangfang_obj_file_path = os.path.join(basepath, '受让方_NLPCLSModel.obj')
shourangfang_model_file_path = os.path.join(basepath, '受让方_LR_CO_OCCURRENCE_MATRIX_plus_0.model')

jzr_obj_file_path = os.path.join(basepath, '基准日_NLPCLSModel.obj')
jzr_model_file_path = os.path.join(basepath, '基准日_LR_WORD2VEC_plus_1.model')

bh_obj_file_path = os.path.join(basepath, '编号_NLPCLSModel.obj')
bh_model_file_path = os.path.join(basepath, '编号_LR_CO_OCCURRENCE_MATRIX_plus_tf_idf_2.model')

fddbr_obj_file_path = os.path.join(basepath, '法定代表人_NLPCLSModel.obj')
fddbr_model_file_path = os.path.join(basepath, '法定代表人_LR_CO_OCCURRENCE_MATRIX_plus_tf_idf_0.model')

fzr_obj_file_path = os.path.join(basepath, '负责人_NLPCLSModel.obj')
fzr__model_file_path = os.path.join(basepath, '负责人_SVC_WORD2VEC_plus_tf_idf_0.model')

rq_obj_file_path = os.path.join(basepath, '日期_NLPCLSModel.obj')
rq__model_file_path = os.path.join(basepath, '日期_LR_CO_OCCURRENCE_MATRIX_plus_tf_idf_0.model')

wyj_obj_file_path = os.path.join(basepath, '违约金_NLPCLSModel.obj')
wyj__model_file_path = os.path.join(basepath, '违约金_LR_CO_OCCURRENCE_MATRIX_plus_1.model')

zrjk_obj_file_path = os.path.join(basepath, '转让价款_NLPCLSModel.obj')
zrjk__model_file_path = os.path.join(basepath, '转让价款_LR_WORD2VEC_plus_tf_idf_1.model')

xiyi_obj_file_path = os.path.join(basepath, '协议_NLPCLSModel.obj')
xiyi__model_file_path = os.path.join(basepath, '协议_SVC_CO_OCCURRENCE_MATRIX_mul_0.model')

zhushuo_obj_file_path = os.path.join(basepath, '住所_NLPCLSModel.obj')
zhushuo__model_file_path = os.path.join(basepath, '住所_LR_WORD2VEC_plus_0.model')

dizhi_obj_file_path = os.path.join(basepath, '地址_NLPCLSModel.obj')
dizhi__model_file_path = os.path.join(basepath, '地址_LR_CO_OCCURRENCE_MATRIX_plus_0.model')

bzj_obj_file_path = os.path.join(basepath, '保证金_NLPCLSModel.obj')
bzj__model_file_path = os.path.join(basepath, '保证金_LR_CO_OCCURRENCE_MATRIX_plus_2.model')
...
def output_elements_from_docx(docx_file_path):
    ...
    top = pikeup(zhuanrangfang_obj_file_path,
                 zhuanrangfang_model_file_path,
                 docx_file_path, 5)
    if top:
        string = top[0][-1]
        k_s = 0
    ...
    top = pikeup(shourangfang_obj_file_path,
                 shourangfang_model_file_path,
                 docx_file_path, 5)
    if top:
        string = top[0][-1]
        k_s = 0
    ...
    #
    #         # 将预测结果按 0 类别 (非合同要素句子) 概率由小到大进行排序
    #
    ...
    # 合同类型
    tmp = pikeup(xiyi_obj_file_path,
                 xiyi__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        business_types = tmp[0][-1]
    else:
        business_types = ''

    # 协议签订日期
    tmp = pikeup(rq_obj_file_path,
                 rq__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        data_of_agreement = tmp[0][-1]
    else:
        data_of_agreement = ''
    ...
    debator = ''
    # 转让方负责人
    tmp = pikeup(fzr_obj_file_path,
                 fzr__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        transferor_responsible = tmp[0][-1]
    else:
        transferor_responsible = ''

    # 受让方法定代表人
    tmp = pikeup(fddbr_obj_file_path,
                 fddbr_model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        recipie_legal = tmp[0][-1]
    else:
        recipie_legal = ''
    ...
    debator_responsible = ''
    # 转让方住所
    tmp = pikeup(zhushuo_obj_file_path,
                 zhushuo__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        transferor_location = tmp[0][-1]
    else:
        transferor_location = ''

    # 受让方住所
    tmp = pikeup(dizhi_obj_file_path,
                 dizhi__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        recipie_location = tmp[0][-1]
    else:
        recipie_location = ''
    ...
    # 转让价款
    tmp = pikeup(zrjk_obj_file_path,
                 zrjk__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        transfer_price = tmp[0][-1]
    else:
        transfer_price = ''
    ...
    creditor_money_list = ['', '']
    # 违约金
    tmp = pikeup(wyj_obj_file_path,
                 wyj__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        breach_of_contract = tmp[0][-1]
    else:
        breach_of_contract = ''
    ...
    transferor_account = ''
    # 交易保证金详情
    tmp = pikeup(bzj_obj_file_path,
                 bzj__model_file_path,
                 docx_file_path, 5)
    if len(tmp):
        trade_margin_details = tmp[0][-1]
    else:
    ...
    for i_line_num in range(docx_text_list_lens):
        ...
        #     if (i + 2 < string_lens and string[i] == '编' and string[i + 1] == '号' and (
        #             string[i + 2] == '：' or string[i + 2] == ':') and string[-1] == '号'):
        #         if contract_no == '':
        #             contract_no = string
        #             break
        #
        ...
        # # 提取协议签订日期
        # for i in range(string_lens):
        #     if i + 6 < string_lens and string[i] == '协' and string[i + 1] == '议' and string[i + 2] == '签' and string[
        #         i + 3] == '订' and string[i + 4] == '日' and string[i + 5] == '期' and (
        #             string[i + 6] == '：' or string[i + 6] == ':'):
        #         if i < max_lens_da:
        #             max_lens_da = i
        ...
        for i in range(string_lens):
            if i + 6 < string_lens and string[i] == '协' and string[i + 1] == '议' and string[i + 2] == '签' and string[
                i + 3] == '订' and string[i + 4] == '地' and string[i + 5] == '点' and (
                    string[i + 6] == '：' or string[i + 6] == ':'):
                if i < max_lens_ps:
                    max_lens_ps = i
        ...
        # 提取债务方
        for i in range(string_lens):
            if i + 3 < string_lens and string[i] == '债' and string[i + 1] == '务' and (
                    string[i + 2] == '方' or string[i + 2] == '人') and (string[i + 3] == '：' or string[i + 3] == ':'):
        ...
        # 提取转让方负责人/法定代表人
        # for i in range(string_lens):
        #     if (i + 3 < string_lens and string[i] == '负' and string[i + 1] == '责' and (
        #             string[i + 2] == '方' or string[i + 2] == '人') and (
        #                 string[i + 3] == '：' or string[i + 3] == ':')) or (
        #             i + 5 < string_lens and string[i] == '法' and string[i + 1] == '定' and string[i + 2] == '代' and
        #             string[i + 3] == '表' and string[i + 4] == '人' and (string[i + 5] == '：' or string[i + 5] == ':')):
        #         if i_line_num - 1 >= 0:
        #             string_pre = docx_text_string_list[i_line_num - 1]
        #             string_pre_lens = len(string_pre)
        #             for k in range(string_pre_lens):
        #                 if (k + 1 < string_pre_lens and string_pre[k] == '甲' and string_pre[k + 1] == '方') or (
        #                         k + 2 < string_pre_lens and string_pre[k] == '转' and string_pre[k + 1] == '让' and
        #                         string_pre[k + 2] == '方'):
        ...
        #             string_pre_lens = len(string_pre)
        #             for k in range(string_pre_lens):
        #                 if (k + 1 < string_pre_lens and string_pre[k] == '乙' and string_pre[k + 1] == '方') or (
        #                         k + 2 < string_pre_lens and string_pre[k] == '受' and string_pre[k + 1] == '让' and
        #                         string_pre[k + 2] == '方'):
        ...
        # 提取转让方住所
        # for i in range(string_lens):
        #     if (i + 2 < string_lens and string[i] == '住' and string[i + 1] == '所' and (
        #             string[i + 2] == '：' or string[i + 2] == ':')) or (
        #             i + 2 < string_lens and string[i] == '地' and string[i + 1] == '址' and (
        #             string[i + 2] == '：' or string[i + 2] == ':')):
        #         if i_line_num - 2 >= 0:
        #             string_pre = docx_text_string_list[i_line_num - 2]
        #             string_pre_lens = len(string_pre)
        #             for k in range(string_pre_lens):
        #                 if (k + 2 < string_pre_lens and string_pre[k] == '转' and string_pre[k + 1] == '让' and
        #                     string_pre[k + 2] == '方') or (
        #                         k + 1 < string_pre_lens and string_pre[k] == '甲' and string_pre[k + 1] == '方'):
        ...
        #             string_pre_lens = len(string_pre)
        #             for k in range(string_pre_lens):
        #                 if (k + 2 < string_pre_lens and string_pre[k] == '受' and string_pre[k + 1] == '让' and
        #                     string_pre[k + 2] == '方') or (
        #                         k + 1 < string_pre_lens and string_pre[k] == '乙' and string_pre[k + 1] == '方'):
        ...
        # 提取账面本金余额
        for i in range(string_lens):
            if (i + 5 < string_lens and string[i] == '账' and string[i + 1] == '面' and string[i + 2] == '本' and string[
                i + 3] == '金' and string[i + 4] == '余' and string[i + 5] == '额'):
                b_loc = -1
        ...
        # 提取利息
        for i in range(string_lens):
            if (i + 2 < string_lens and string[i] == '利' and string[i + 1] == '息' and (
                    string[i + 2] == '为' or string[i + 2] == '是')):
                b_loc = -1
                for b in range(string_lens):
                    if string[b] == '币':
                        b_loc = b
        ...
        # 提取其他债权
        for i in range(string_lens):
            if (i + 3 < string_lens and string[i] == '其' and string[i + 1] == '他' and string[i + 2] == '债' and string[
                i + 3] == '权'):
                b_loc = -1
                for b in range(string_lens):
                    if string[b] == '币':
        ...
        # 提取整体债权
        for i in range(string_lens):
            if (i + 3 < string_lens and string[i] == '整' and string[i + 1] == '体' and string[i + 2] == '债' and string[
                i + 3] == '权'):
                b_loc = -1
                for b in range(string_lens):
                    if string[b] == '币':
        ...
        # # 提取转让价款
        # for i in range(string_lens):
        #     if (i + 3 < string_lens and string[i] == '转' and string[i + 1] == '让' and string[i + 2] == '价' and string[
        #         i + 3] == '款'):
        #         b_loc = -1
        #         for b in range(string_lens):
        #             if string[b] == '币':
        ...
        # 提取债权金额
        for i in range(string_lens):
            if (i + 3 < string_lens and string[i] == '债' and string[i + 1] == '权' and string[i + 2] == '金' and string[
                i + 3] == '额'):
                b_loc = -1
                for b in range(string_lens):
                    if string[b] == '币':
        ...
        # 提取本息总额
        for i in range(string_lens):
            if (i + 3 < string_lens and string[i] == '本' and string[i + 1] == '息' and string[i + 2] == '总' and string[
                i + 3] == '额'):
                b_loc = -1
                y_loc = -1
                for b in range(string_lens):
        ...
        # 提取本金余额
        for i in range(string_lens):
            if (i + 3 < string_lens and string[i] == '本' and string[i + 1] == '金' and string[i + 2] == '余' and string[
                i + 3] == '额'):
                b_loc = -1
                y_loc = -1
                q_loc = -1
        ...
        # 提取转让方开户银行
        for i in range(string_lens):
            if (i + 4 < string_lens and string[i] == '开' and string[i + 1] == '户' and string[i + 2] == '银' and string[
                i + 3] == '行' and (string[i + 4] == '：' or string[i + 4] == ':')) or (
                    i + 3 < string_lens and string[i] == '开' and string[i + 1] == '户' and string[i + 2] == '行' and (
                    string[i + 3] == '：' or string[i + 3] == ':')):
                z_loc = -1
                if i_line_num - 1 >= 0:
                    string_pre = docx_text_string_list[i_line_num - 1]
                    string_pre_lens = len(string_pre)
        ...
        # 提取转让方户名
        for i in range(string_lens):
            if (i + 2 < string_lens and string[i] == '户' and string[i + 1] == '名' and (
                    string[i + 2] == '：' or string[i + 2] == ':')):
                z_loc = -1
                if i_line_num - 2 >= 0:
                    string_pre = docx_text_string_list[i_line_num - 2]
        ...
        # 提取转让方账户
        for i in range(string_lens):
            if (i + 2 < string_lens and string[i] == '账' and (string[i + 1] == '户' or string[i + 1] == '号') and (
                    string[i + 2] == '：' or string[i + 2] == ':')):
                z_loc = -1
                if i_line_num - 3 >= 0:
        ...
        # 提取交易保证金
        # for i in range(string_lens):
        #     if (i + 1 < string_lens and string[i] == '交' and string[i + 1] == '易' and string[i + 2] == '保' and string[
        #         i + 3] == '证' and string[i + 4] == '金'):
        #         b_loc = -1
        #         y_loc = -1
        #         for b in range(string_lens):
        #             if string[b] == '币':
    ...
    if string_k != '':
        ...
        # 找到句子的开始、结束标记
        for i in range(string_lens):
            if i + 5 < string_lens and string_k[i] == '账' and string_k[i + 1] == '面' and string_k[i + 2] == '本' and \
                    string_k[i + 3] == '金' and string_k[i + 4] == '余' and string_k[i + 5] == '额':
        ...
        # 提取小写金额
        for i in range(sentence_start_loc, sentence_end_loc):
            if i + 2 <= sentence_end_loc and string_k[i] == '小' and string_k[i + 1] == '写' and (
                    string_k[i + 2] == '：' or string_k[i + 2] == ':'):
                x_start_loc = i + 2
                break
        for i in range(sentence_start_loc, sentence_end_loc):
            if string_k[i] == '元':
        ...
        # elements_dict['账面本金余额'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['账面本金余额'] = account_remain_money_list
        # elements_dict['账面本金余额'] = account_remain_money
    ...
    if string_k != '':
        ...
        for i in range(string_lens):
            if i + 1 < string_lens and string_k[i] == '利' and string_k[i + 1] == '息':
                ...
                break
        for i in range(string_lens):
            if i + 3 < string_lens and string_k[i] == '其' and string_k[i + 1] == '他' and string_k[i + 2] == '债' and \
                    string_k[i + 3] == '权':
                sentence_end_loc = i
                break
        ...
        # elements_dict['利息'] = '大写：' + d_string + '  小写：' + x_string
    else:
        elements_dict['利息'] = interest_list
        # elements_dict['利息'] = interest
    ...
    if string_k != '':
        ...
        for i in range(string_lens):
            if i + 3 < string_lens and string_k[i] == '其' and string_k[i + 1] == '他' and string_k[i + 2] == '债' and \
                ...
                break
        for i in range(string_lens):
            if i + 3 < string_lens and string_k[i] == '整' and string_k[i + 1] == '体' and string_k[i + 2] == '债' and \
                    string_k[i + 3] == '权':
                sentence_end_loc = i
                break
        ...
        # elements_dict['其他债权'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['其他债权'] = other_creditors_list
        # elements_dict['其他债权'] = other_creditors
    ...
    if string_k != '':
        ...
        # elements_dict['整体债权'] = '大写：' + d_string + '  小写：' + x_string


    else:
        elements_dict['整体债权'] = total_creditors_list
        # elements_dict['整体债权'] = total_creditors
    ...
    if string_k != '':
        ...
        # elements_dict['转让价款'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['转让价款'] = transfer_price_list
        # elements_dict['转让价款'] = transfer_price
    ...
    if string_k != '':
        ...
        for i in range(0, sentence_end_loc):
            if i + 3 <= sentence_end_loc and string_k[i] == '债' and string_k[i + 1] == '权' and string_k[
                i + 2] == '金' and string_k[i + 3] == '额':
                sentence_start_loc = i
                break
        ...
        for i in range(sentence_start_loc, sentence_end_loc):
            if i + 1 <= sentence_end_loc and string_k[i] == '元' and (
                    string_k[i + 1] == '，' or string_k[i + 1] == ',' or string_k[i + 1] == '。'):
        ...
        # elements_dict['债权金额'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['债权金额'] = creditor_money_list
        # elements_dict['债权金额'] = creditor_money
    ...
    if string_k != '':
        ...
        # elements_dict['违约金'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['违约金'] = breach_of_contract_list
        # elements_dict['违约金'] = breach_of_contract
    ...
    if string_k != '':
        ...
        sentence_end_loc = string_lens
        for i in range(string_lens):
            if i + 3 < string_lens and string_k[i] == '本' and string_k[i + 1] == '息' and string_k[i + 2] == '总' and \
                    string_k[i + 3] == '额':
                sentence_start_loc = i
                break
        for i in range(sentence_start_loc, sentence_end_loc):
        ...
        # elements_dict['本息总额'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['本息总额'] = total_price_and_interest_list
        # elements_dict['本息总额'] = total_price_and_interest
    ...
    if string_k != '':
        ...
        sentence_end_loc = string_lens
        for i in range(string_lens):
            if i + 3 < string_lens and string_k[i] == '本' and string_k[i + 1] == '金' and string_k[i + 2] == '余' and \
                    string_k[i + 3] == '额':
                sentence_start_loc = i
                break
        for i in range(sentence_start_loc, sentence_end_loc):
        ...
        # elements_dict['本金余额'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['本金余额'] = pricipal_remain_list
        # elements_dict['本金余额'] = pricipal_remain
    ...
    if string_k != '':
        ...
        # elements_dict['欠息'] = '大写：' + d_string + '  小写：' + x_string

    else:
        elements_dict['欠息'] = debit_interest_list
        # elements_dict['欠息'] = debit_interest
    ...
    if string_k != '':
        ...
        loc = 0
        for i in range(string_lens):
            if i + 1 < string_lens and (string_k[i] == '户' or string_k[i] == '号') and (
                    string_k[i + 1] == '：' or string_k[i + 1] == ':'):
                loc = i + 1
                break
        if loc + 1 > string_lens:
            loc -= 1
...
def sentences_to_vector(txt_bow_file_name_, all_sentences):
    ...
    # all_sentences_new_list = [['s1_1','s1_2','s1_1'],[]]
    # all_sentences_new_set = [('s1_1','s1_2'),()]
...
def check_equal(money_list):
    ...
    # print(d_string, '  ', x_string_new)

    money_x = [m for m in money_list[1] if m.isdigit()]
...
def check_res(elements_dict):
    ...
    for element in elements_dict:
        if element == '账面本金余额' or element == '利息' or element == '其他债权' or element == '整体债权' or element == '转让价款' or element == '债权金额' or element == '违约金' or element == '本息总额' or element == '本金余额' or element == '欠息':
            print(element)
            if elements_dict[element] and elements_dict[element] != ['', ''] :
                check_res_dict[element] = check_amount(elements_dict[element][0], elements_dict[element][1])
            else:
                check_res_dict[element] = ''

    # for item in check_res_dict:
    #     print(item,'  ',check_res_dict[item])
